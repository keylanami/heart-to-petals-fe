// DATA TOKO (TENANTS)
export const SHOPS = [
  {
    id: 101,
    name: "Rosy Garden Bandung",
    location: "Dago, Bandung",
    image: "/assets/flowershop/rosy_garden.png", 
    rating: 4.9,
    reviewCount: 1240, 
    distance: "1.2 km", 
    can_customize: true, 
    openTime: "08.00 - 20.00"
  },
  {
    id: 102,
    name: "Eternal Florist Jakarta",
    location: "Kemang, Jakarta Selatan",
    image: "/assets/flowershop/eternal_florist.png",
    rating: 4.7,
    reviewCount: 850,
    distance: "3.5 km",
    can_customize: false, 
    openTime: "09.00 - 21.00"
  },
  {
    id: 103,
    name: "The Flower Studio",
    location: "Cikini, Jakarta Pusat",
    image: "/assets/flowershop/the_flower_studio.png",
    rating: 4.8,
    reviewCount: 210,
    distance: "0.8 km",
    can_customize: true, 
    openTime: "10.00 - 18.00"
  },
  {
    id: 104,
    name: "Sentra Bunga Rawabelong",
    location: "Kebon Jeruk, Jakarta Barat",
    image: "/assets/flowershop/sentara_bunga.png",
    rating: 4.5,
    reviewCount: 5300, 
    distance: "12 km",
    can_customize: false,
    openTime: "24 Jam"
  },
  {
    id: 105,
    name: "Luxe De Pere En Fils",
    location: "SCBD, Jakarta Selatan",
    image: "/assets/flowershop/de_pere.png",
    rating: 5.0,
    reviewCount: 45, 
    distance: "4.2 km",
    can_customize: true,
    openTime: "09.00 - 17.00"
  },
  {
    id: 106,
    name: "Petal & Paper",
    location: "BSD City, Tangerang",
    image: "/assets/flowershop/petal_paper.png",
    rating: 4.6,
    reviewCount: 112,
    distance: "25 km",
    can_customize: true,
    openTime: "08.00 - 20.00"
  }
];




// DATA PRODUK (allItems)
export const allItems = [
  {
    id: 1,
    shop: SHOPS[0],
    type: "product",
    title: "Head Over Heels",
    price: 850000,
    image: "/assets/bouquet/peace/crimson_promise.png",
    category: "Warm",
    tag: "Romance",
    theme: "light",
    isBestSeller: true,
    desc: "Cinta yang meledak-ledak dan penuh afeksi. Cocok untuk menyatakan perasaan yang mendalam.",
    story: "Terinspirasi dari kisah cinta klasik yang tak lekang oleh waktu. Merah melambangkan keberanian mencintai.",
    care: "Potong ujung batang 1cm setiap hari, ganti air dingin, dan jauhkan dari sinar matahari langsung.",
    composition: [
        "Mawar Merah (Red Rose) - 10 tangkai",
        "Baby Breath - Filler",
        "Daun Ruscus - Greenery",
        "Pita Satin Premium Maroon"
    ]
  },
  {
    id: 2,
    shop: SHOPS[0],
    type: "product",
    title: "Sunset Harmony",
    price: 450000,
    image: "/assets/bouquet/peace/sunset_harmony.png", 
    category: "Peace",
    tag: "Friendship",
    theme: "light",
    isBestSeller: false,
    desc: "Kesegaran pagi hari yang membawa ketenangan.",
    story: "Diciptakan untuk menyapa teman lama dengan kehangatan yang tidak berlebihan.",
    care: "Semprotkan air (mist) ke kelopak bunga tulip agar tetap segar.",
    composition: [
        "Tulip Putih - 5 tangkai",
        "Lily of the Valley - 3 tangkai",
        "Kertas Wrapping Transparan"
    ]
  },
  {
    id: 3,
    shop: SHOPS[0], 
    type: "product",
    title: "Bright Future",
    price: 450000,
    image: "/assets/bouquet/peace/sunset_harmony.png",
    category: "Warm", tag: "Graduation", desc: "Menyambut masa depan cerah dengan senyuman.", theme: "light",
  },
  {
    id: 4,
    shop: SHOPS[1], 
    type: "product",
    title: "Sweet Gratitude",
    price: 350000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Thank You", desc: "Terima kasih yang manis untuk dia yang spesial.", theme: "light",
  },
  {
    id: 5,
    shop: SHOPS[1], 
    type: "product",
    title: "Silent Tears",
    price: 700000,
    image: "/assets/bouquet/peace/crimson_promise.png",
    category: "Gloomy", tag: "Grief", desc: "Penghormatan terakhir untuk jiwa yang tenang.", theme: "dark",
  },
  { id: "promo-1", type: "promo" },
  {
    id: 6,
    shop: SHOPS[0], 
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/regret/first_date_bloom.png",
    category: "Gloomy", tag: "Regret", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark", isBestSeller: true
  },
  {
    id: 7,
    shop: SHOPS[1], 
    type: "product",
    title: "Silent Tears",
    price: 700000,
    image: "/assets/bouquet/peace/crimson_promise.png",
    category: "Gloomy", tag: "Grief", desc: "Penghormatan terakhir untuk jiwa yang tenang.", theme: "dark",
  },
  {
    id: 8,
    shop: SHOPS[0], 
    type: "product",
    title: "Sweet Gratitude",
    price: 350000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Thank You", desc: "Terima kasih yang manis untuk dia yang spesial.", theme: "light",
  },
  {
    id: 9,
    shop: SHOPS[1], 
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/regret/first_date_bloom.png",
    category: "Gloomy", tag: "Regret", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark",
  },
  {
    id: 10,
    shop: SHOPS[0], 
    type: "product",
    title: "Sweet Gratitude",
    price: 350000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Thank You", desc: "Terima kasih yang manis untuk dia yang spesial.", theme: "light",
  },
  {
    id: 11,
    shop: SHOPS[1], 
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/regret/first_date_bloom.png",
    category: "Gloomy", tag: "Regret", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark",
  },
  {
    id: 12,
    shop: SHOPS[0],
    type: "product",
    title: "Silent Tears",
    price: 700000,
    image: "/assets/bouquet/peace/crimson_promise.png",
    category: "Gloomy", tag: "Grief", desc: "Penghormatan terakhir untuk jiwa yang tenang.", theme: "dark",
  },
  {
    id: 13,
    shop: SHOPS[1],
    type: "product",
    title: "Sweet Gratitude",
    price: 350000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Thank You", desc: "Terima kasih yang manis untuk dia yang spesial.", theme: "light", isBestSeller: true
  },
  { id: "promo-2", type: "promo" },
  {
    id: 14,
    shop: SHOPS[0],
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/regret/first_date_bloom.png",
    category: "Gloomy", tag: "Regret", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark",
  },
  {
    id: 15,
    shop: SHOPS[1],
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Peace", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark",
  },
  {
    id: 16,
    shop: SHOPS[0],
    type: "product",
    title: "Midnight Regret",
    price: 480000,
    image: "/assets/bouquet/peace/moonlight_serenity.png",
    category: "Warm", tag: "Peace", desc: "Penyesalan terdalam di tengah malam yang sunyi.", theme: "dark",
  },
  {
  id: 17,
  shop: SHOPS[0],
  type: "product",
  title: "Garden Symphony",
  price: 820000,
  image: "/assets/bouquet/romance/garden_syphony.png",
  category: "Warm", tag: "Romance", desc: "Harmoni cinta yang tumbuh alami, lembut namun penuh makna.", theme: "light",
  },
  {
  id: 18,
  shop: SHOPS[0],
  type: "product",
  title: "Midnight Velvet",
  price: 900000,
  image: "/assets/bouquet/romance/midnight_velvet.png",
  category: "Warm", tag: "Romance", desc: "Cinta yang dalam, misterius, dan penuh daya tarik.", theme: "dark",
  },
  {
  id: 19,
  shop: SHOPS[1],
  type: "product",
  title: "Peony Dream",
  price: 780000,
  image: "/assets/bouquet/romance/peony_dream.png",
  category: "Warm", tag: "Romance", desc: "Mimpi manis tentang cinta yang lembut dan anggun.", theme: "light",
  },
  {
  id: 20,
  shop: SHOPS[1],
  type: "product",
  title: "Sweet Harmony",
  price: 760000,
  image: "/assets/bouquet/romance/sweet_harmony.png",
  category: "Warm", tag: "Romance", desc: "Keselarasan cinta yang sederhana namun menenangkan.", theme: "light",
  },
  {
  id: 21,
  shop: SHOPS[0],
  type: "product",
  title: "Autumn Blooms",
  price: 620000,
  image: "/assets/bouquet/gratitude/autumn_blooms.png",
  category: "Warm", tag: "Gratitude", desc: "Ucapan terima kasih yang hangat seperti musim gugur.", theme: "light",
  },
  {
  id: 22,
  shop: SHOPS[1],
  type: "product",
  title: "Sol De Mi Vida",
  price: 650000,
  image: "/assets/bouquet/gratitude/sol_de_mi_vida.png",
  category: "Warm", tag: "Appreciation", desc: "Kamu adalah matahari dalam hidupku.", theme: "light",
  },
  {
  id: 23,
  shop: SHOPS[1],
  type: "product",
  title: "Sunshine Bliss",
  price: 600000,
  image: "/assets/bouquet/gratitude/sunshine_bliss.png",
  category: "Warm", tag: "Thank You", desc: "Kebahagiaan sederhana yang penuh kehangatan.", theme: "light",
  },
];

