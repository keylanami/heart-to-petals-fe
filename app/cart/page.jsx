  ...
  return (
    <main className="bg-[#F5F5F5] min-h-screen pb-32">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-32">
        <h1 className="text-3xl font-serif font-bold text-dark-green mb-8">Shopping Cart</h1>
                ...
                <div key={group.shop?.id || 'unknown'} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  ...
                  <div className="bg-white p-4 border-b border-gray-100 flex items-center gap-3">
                      <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-dark-green">{group.shop?.name || "Unknown Shop"}</span>
                      </div>
                      {group.shop?.location && (
                      ...
                      {group.items.map((item) => (
                          <div key={item.id} className="flex gap-4 items-start">
                              ...
                              <div className="w-24 h-24 bg-gray-100 rounded-xl overflow-hidden shrink-0 border border-gray-100">
                                  <img src={item.image || item.previewImage} alt={item.title || item.name} className="w-full h-full object-cover" />
                              </div>
    
                              <div className="flex-1 flex flex-col justify-between min-h-[6rem]">
                                  <div>
                                      <div className="flex justify-between items-start">
                                          <h3 className="font-bold text-gray-800 line-clamp-1">{item.title || item.name}</h3>
                                          <button onClick={() => removeFromCart(item.id)} className="text-gray-300 hover:text-red-500 transition">
                                              <Trash2 size={18} />
                                          </button>
                                      </div>
                                      <p className="text-xs text-gray-400 mt-1">
                                          {item.category ? `${item.category} Mood` : 'Custom Bouquet'} • {item.tag || 'Handmade'}
                                      </p>
                                  </div>
    
                                  <div className="flex justify-between items-end mt-4">
                                      <span className="font-sans font-bold text-dark-green">{toRupiah(item.price)}</span>
                                      
                                      <div className="flex items-center gap-3 bg-gray-50 rounded-full px-3 py-1 border border-gray-200">
                                          <button onClick={() => decreaseQty(item)} className="text-gray-400 hover:text-dark-green text-xs">
                                              <Minus size={14}/>
                                          </button>
                                          <span className="text-sm font-bold w-4 text-center">{item.qty}</span>
                                          <button onClick={() => addToCart(item, 1)} className="text-gray-400 hover:text-dark-green text-xs">
                                              <Plus size={14}/>
                                          </button>
                                      </div>
                                  </div>
