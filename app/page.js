...
import { Search, MapPin, Star, ArrowRight, Sparkles, Store } from "lucide-react";
...
const DashboardProductCard = ({ item }) => {
  return (
    <Link href={`/product/${item.id}`} className="group relative flex-shrink-0 w-[260px] md:w-[300px] h-[480px] cursor-pointer mr-8">
      ...
      <section className="relative pt-36 pb-10 px-4 md:px-6 overflow-hidden">
         <div className="max-w-7xl mx-auto relative z-10">
            <div className="flex justify-between items-end mb-6 px-2">
                <div>
                    <p className="text-sage-green text-xs font-bold tracking-widest uppercase mb-3">Welcome back, Kei</p>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >   
                        <div className="absolute inset-0 opacity-40 mix-blend-overlay">
                             <img src={PROMOS[currentPromo].image} className="w-full h-full object-cover" />
                        </div>
                        <Snowfall />
                        <div className="relative z-10 px-8 md:px-16 max-w-2xl">
                            <span className={`text-xs font-bold tracking-[0.2em] uppercase border border-current px-3 py-1 rounded-full mb-4 inline-block ${PROMOS[currentPromo].text}`}>Special Offer</span>
                            <h2 className={`text-4xl md:text-7xl font-serif font-bold mb-6 leading-tight ${PROMOS[currentPromo].text}`}>{PROMOS[currentPromo].title}</h2>
                            <p className={`text-lg md:text-xl opacity-90 mb-10 font-light ${PROMOS[currentPromo].text}`}>{PROMOS[currentPromo].subtitle}</p>
                            <Link href="/toko" className="bg-white text-dark-green px-8 py-3.5 rounded-full font-bold shadow-lg hover:bg-cream-bg transition inline-flex items-center gap-2">Shop Now <ArrowRight size={16} /></Link>
                        </div>
                    </motion.div>
                </AnimatePresence>
                <div className="absolute bottom-6 left-0 w-full flex justify-center gap-2 z-20">
                    {PROMOS.map((_, idx) => (
                        <button key={idx} onClick={() => setCurrentPromo(idx)} className={`w-2 h-2 rounded-full transition-all duration-300 ${currentPromo === idx ? "w-8 bg-white" : "bg-white/50"}`} />
                    ))}
                </div>
            </div>
         </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 pt-10 pr-6 pl-10">
                <div className="relative">
                    ...
                    <h2 className="text-4xl md:text-5xl font-serif font-bold text-dark-green mb-2">
                        Curated <span className="italic font-light text-sage-green">Favorites</span>
                    </h2>
         ...
         <div className="max-w-7xl mx-auto md:px-6">
            <div className="flex justify-between items-end mb-8 pr-6">
                <div>
                    ...
                    <h2 className="text-3xl font-serif font-bold text-dark-green">Florist Terdekat</h2>
                </div>
            </div>
