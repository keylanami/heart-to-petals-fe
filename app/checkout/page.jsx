...
import { useCart } from "@/app/context/CartContext"; // Sesuaikan path context
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { MapPin, Store, Truck, ChevronRight, CreditCard, Banknote } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  // State untuk menyimpan Pilihan Pengiriman per Toko
  // Format: { "101": { name: "Reguler", cost: 15000 }, "102": { ... } }
  const [shippingSelection, setShippingSelection] = useState({});

  // 1. Load Data Barang yang Dipilih
  useEffect(() => {
    const selectedIds = JSON.parse(localStorage.getItem("checkoutIds") || "[]");
    
    // Filter item di keranjang yang ID-nya ada di selectedIds
    const itemsToBuy = cart.filter(item => selectedIds.includes(item.id));
    setCheckoutItems(itemsToBuy);

    // Set Default Shipping (Reguler) untuk setiap toko yang ada di list
    const initialShipping = {};
    itemsToBuy.forEach(item => {
        const shopId = item.shop?.id || 'unknown';
        if (!initialShipping[shopId]) {
            initialShipping[shopId] = { name: "Reguler", cost: 12000, eta: "3-5 Hari" };
        }
    });
  ...
  // --- LOGIC HITUNG TOTAL ---
  const subTotalProduct = checkoutItems.reduce((sum, item) => sum + (item.price * item.qty), 0);
  
  const totalShippingCost = Object.values(shippingSelection).reduce((sum, option) => sum + option.cost, 0);
  
  const grandTotal = subTotalProduct + totalShippingCost + 1000; // +1000 Biaya Layanan
  ...
  const handlePlaceOrder = () => {
    ...
    // 2. (Opsional) Hapus item yang dibeli dari CartContext (biar keranjang bersih)
    
    // 3. Simulasi Loading sebentar biar kerasa "mikir"
      ...
      <div className="max-w-6xl mx-auto px-4 pt-32 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {Object.entries(groupedCheckout).map(([shopId, group]) => (
                <div key={shopId} className="bg-white p-6 rounded-xl shadow-sm">
                    ...
                    <div className="flex items-center gap-2 mb-4 pb-4 border-b border-gray-100">
                        ...
                        <span className="font-bold text-gray-800">{group.shop?.name || "Unknown Shop"}</span>
                    </div>
                        ...
                        {group.items.map((item) => (
                            <div key={item.id} className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-gray-400 mb-1">{item.category || "Custom"}</p>
                                    <div className="flex justify-between items-center">
                                        <p className="text-sm text-gray-500">{item.qty} x {toRupiah(item.price)}</p>
                                        <p className="font-bold text-dark-green">{toRupiah(item.price * item.qty)}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="mt-6 pt-4 border-t border-dashed border-gray-200 bg-blue-50/50 -mx-6 px-6 pb-2">
                        ...
                        <div className="mt-4 flex items-center gap-3">
                            <label className="text-xs text-gray-500 whitespace-nowrap">Pesan:</label>
                            <input type="text" placeholder="(Opsional) Tinggalkan pesan ke penjual..." className="w-full text-sm border-b border-gray-300 bg-transparent focus:outline-none focus:border-dark-green py-1" />
                        </div>
                    </div>
                </div>
            ))}
        ...
        <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-sm sticky top-28">
                <h3 className="font-bold text-lg mb-4 text-dark-green">Ringkasan Pesanan</h3>
                ...
                <div className="mb-6">
                    <button className="w-full flex justify-between items-center p-3 border border-gray-200 rounded-lg hover:border-dark-green transition group">
                        <div className="flex items-center gap-3">
                            <CreditCard size={18} className="text-gray-500 group-hover:text-dark-green" />
                            <span className="text-sm font-bold text-gray-700">Pilih Pembayaran</span>
                        </div>
                ...
                <div className="space-y-3 text-sm text-gray-600 border-b border-gray-100 pb-4 mb-4">
                    <div className="flex justify-between">
                        <span>Total Harga ({checkoutItems.reduce((acc, i) => acc + i.qty, 0)} barang)</span>
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="font-bold text-xl text-dark-green">{toRupiah(grandTotal)}</span>
                </div>
