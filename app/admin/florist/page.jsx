...
import { 
  LayoutDashboard, Package, Palette, ShoppingBag, Plus, Check, X, DollarSign, User, ArrowLeft 
} from "lucide-react";
...
export default function FloristAdminPage() {
  // --- 1. DECLARE SEMUA HOOKS DI PALING ATAS (WAJIB URUT) ---
  ...
  // --- 3. DATA PROCESSING (SAFE MODE) ---
  // Kita hitung ini dulu, tapi pake safe check (?.) biar ga error kalau user null
  const SHOP_ID = user?.shop?.id; 

  const myOrders = SHOP_ID 
    ? orders.filter(order => order.items.some(item => item.shop?.id === SHOP_ID))
    : [];

  const myInventory = SHOP_ID 
    ? inventory.filter(item => item.shopId === SHOP_ID)
    : [];
  ...
  const handleAddProduct = (e) => {
    ...
    addItem({
        ...
        id: Date.now(),
        shopId: SHOP_ID,
        shop: { name: user.shop.name, id: SHOP_ID },
    ...
    showToast("Item berhasil ditambahkan ke tokomu!", "success");
  };

  // --- 5. RENDER GUARD (BARU BOLEH RETURN DI SINI) ---
  if (!user || !user.shop) {
      return <div className="min-h-screen bg-cream-bg flex items-center justify-center text-gray-400">Loading Admin...</div>;
  }
  ...
  const OverviewView = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
             <div className="flex items-center gap-4">
               ...
               <div>
                 <p className="text-gray-500 text-xs font-bold uppercase">Pendapatan Toko</p>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-dark-green mb-4">Log Penjualan ({user.shop.name})</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {myOrders.length === 0 ? (
                    <tr><td colSpan="5" className="text-center py-4 text-gray-400">Belum ada penjualan.</td></tr>
                ) : (
                    myOrders.map((order) => {
                        const myItems = order.items.filter(i => i.shop?.id === SHOP_ID);
                        return (
                        <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="px-4 py-3 font-bold font-mono">#{order.id}</td>
                            <td className="px-4 py-3">{order.customer}</td>
                            <td className="px-4 py-3 text-xs text-gray-500">
                                {myItems.map(i => `${i.title} (x${i.qty||1})`).join(", ")}
                            </td>
                            <td className="px-4 py-3">
                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase 
                                ${order.status === 'Completed' ? 'bg-green-100 text-green-600' : 
                                order.status === 'waiting_approval' ? 'bg-yellow-100 text-yellow-600' : 
  ...
  const RequestsView = () => {
    const pendingOrders = myOrders.filter(o => o.status === "waiting_approval");
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <p className="font-bold mb-1">Items yang perlu di-approve:</p>
                    <ul className="list-disc list-inside">
                        {order.items.filter(i => i.shop?.id === SHOP_ID).map((item, idx) => (
                            <li key={idx}>{item.title} (x{item.qty || 1}) - Custom</li>
                        ))}
                    </ul>
                  </div>
                </div>
                ...
                <div className="flex flex-row md:flex-col justify-center gap-3 min-w-[140px]">
                  <button 
                    onClick={() => {
                        updateOrderStatus(order.id, "payment_pending");
                        showToast("Desain disetujui! Menunggu pembayaran user.", "success");
                    }}
                  ...
                  <button 
                    onClick={() => {
                        ...
                        showToast("Permintaan revisi dikirim.", "info");
                    }}
                    className="flex-1 border border-yellow-200 text-yellow-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-yellow-50 transition flex items-center justify-center gap-2"
                  >
                    <Palette size={16} /> Revisi
                  </button>
                </div>
              </motion.div>
            ))}
  ...
  const InventoryView = () => (
    <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-dark-green">Stok Toko: {user.shop.name}</h2>
          <button 
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {myInventory.length === 0 ? (
                  <tr><td colSpan="4" className="text-center py-8 text-gray-400">Inventory kosong. Tambahkan item baru.</td></tr>
              ) : (
                    ...
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center gap-3">
                          <button onClick={() => updateStock(item.id, -1)} className="w-8 h-8 rounded-xl bg-gray-100 hover:bg-gray-200 font-bold">-</button>
                          <button onClick={() => updateStock(item.id, 1)} className="w-8 h-8 rounded-xl bg-dark-green text-white hover:bg-sage-green font-bold">+</button>
                        </div>
                      </td>
                    </tr>
                  ))
    ...
    <div className="bg-cream-bg min-h-screen font-sans flex">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 fixed h-full z-10 hidden md:flex flex-col">
        ...
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-1">ID: {user.shop.id}</p>
        </div>
            ...
            <Link href="/profile">
                <div className="flex items-center gap-3 mb-4 px-3 rounded-2xl border border-gray-200 p-2 cursor-pointer hover:border-sage-green transition">
                    ...
                    <div className="min-w-0">
                        <p className="text-sm font-bold text-dark-green leading-none mb-1 truncate">{user.shop.name}</p>
      ...
      <main className="flex-1 md:ml-64 p-8">
        <div className="mb-8"><h1 className="text-2xl font-bold capitalize text-dark-green">{activeTab}</h1></div>
          ...
          <div className="fixed inset-0 bg-[#1A2F24]/40 z-50 flex items-center justify-center backdrop-blur-sm p-4">
            ...
            >
                ...
                <div className="relative z-10">
                    <h3 className="font-serif font-bold text-2xl text-dark-green mb-1">Tambah Stok</h3>
                    <p className="text-sm text-gray-500 mb-6">Input item untuk {user.shop.name}</p>
                    
                    <form onSubmit={handleAddProduct} className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Nama Item</label>
                            <input required placeholder="Contoh: Mawar Merah" className="w-full bg-white border border-gray-200 p-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-sage-green focus:border-transparent transition-all shadow-sm" 
                        ...
                        <div className="space-y-1">
                            ...
                            <select className="w-full bg-white border border-gray-200 p-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-sage-green focus:border-transparent transition-all shadow-sm text-gray-700" value={newProduct.type} onChange={e => setNewProduct({...newProduct, type: e.target.value})}>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input required type="number" placeholder="0" className="w-full bg-white border border-gray-200 p-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-sage-green focus:border-transparent transition-all shadow-sm" 
                                    value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: e.target.value})} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Stok Awal</label>
                                <input required type="number" placeholder="0" className="w-full bg-white border border-gray-200 p-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-sage-green focus:border-transparent transition-all shadow-sm" 
                        ...
                        <div className="flex gap-3 pt-6">
                            <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-3.5 bg-white border border-gray-200 text-gray-500 rounded-xl font-bold hover:bg-gray-50 transition shadow-sm">Batal</button>
                            <button type="submit" className="flex-1 py-3.5 bg-dark-green text-white rounded-xl font-bold hover:bg-sage-green shadow-lg transition transform active:scale-95">Simpan</button>
                        </div>
                    </form>
                </div>
            </motion.div>
